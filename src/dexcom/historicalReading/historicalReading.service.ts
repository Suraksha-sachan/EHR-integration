...
@Injectable()
export class OnPatientSyncHistoricalService extends BaseService {
                ...
                org.bot_configurations.map(async (botConfig) => {
                  ...
                  if (collection == 'dexcom_configuration') {
                    ...
                    if (enabled == true) {
                      ...
                      const dexcomAccessTokens = await this.historicalService.getAllDexcomAccessTokens(OrgID);
                          ...
                          dexcomAccessTokens.data.map(async (patientTokens) => {
                            ...
                            if (!patientTokens.historical_sync) {
                              ...
                              if (docsinkPatient.data.length > 0) {
                                ...
                                let readingsData = await this.historicalService.fetchReadings(startDate, endDate, patientTokens.access_token);
                                ...
                                if (readingsData && readingsData.egvs.length > 0) {
                                  ...
                                  await Promise.all(
                                    readingsData.egvs.map(async (readings) => {
                                      ...
                                      const checkDexcomegvsMapping = await this.historicalService.getDexcomEgvs(systemTime, patientTokens.id);
                                      ...
                                      if (checkDexcomegvsMapping.data.length > 0) {
                                        ...
                                        await this.historicalService.updateDexcomEgvs(updateDexcomEgvsData, checkDexcomegvsMapping.data[0].id);
                                      }
                                      else {
                                        const docsinkRpmMetric = await this.docsinkService.createOrUpdateDocsinkMetric(readings, botToken);
                                        ...
                                        await this.historicalService.saveDexcomEgvs(saveDexcomEgvsData);
                                      }
                                  ...
                                  let data = {
                                    statuscode: 201,
                                    endpoint: 'Historical Reading Sync',
                                    method: 'Get',
                              ...
                              await this.historicalService.updatePatientHistoricalSync(patientTokens.id);
                            }
                          })
                        )
